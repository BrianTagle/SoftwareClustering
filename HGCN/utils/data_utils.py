...
def load_data(args, datapath):
    if args.task == 'nc':
        ...
        adj = data['adj_train']
        if args.task == 'lp' or args.task == 'rec':
...
def load_data_junit(data_path):
    object_to_idx = {}
    idx_counter = 0
    edges = []
    with open(os.path.join(data_path, "edge_list.txt"), 'r') as f:
        all_edges = f.readlines()
    for line in all_edges:
        n1, n2 = line.rstrip().split('\t')
        #print(n1,n2)
...
def load_data_linux(data_path):
    ...
    idx_counter = 0
    edges = []
    with open(os.path.join(data_path, "fgraph_I_all.txt"), 'r') as f:
    	all_edges = f.readlines()
    for line in all_edges:
        n1, n2 = line.rstrip().split(' ')
        if n1 in object_to_idx:
